<template>
  <el-row class="app-container">
    ...
    >
      ...
      <el-table-column prop="commit" label="提交信息" show-overflow-tooltip />
      <el-table-column prop="commitSha" label="sha" show-overflow-tooltip />
      <el-table-column prop="type" label="上线" />
      <el-table-column prop="status" label="状态" />
      <el-table-column prop="creator" label="提交于" />
      <el-table-column prop="editor" label="发布于" />
...
export default {
  ...
  created() {
    getProject().then((response) => {
      this.projectOption = response.data.projectList
    })
    this.getDeploy()
  },
  methods: {
    getDeploy() {
      getDeploy().then((response) => {
        const deployList = response.data.deployList
    ...
    selectProject() {
      branch(this.form.projectId).then((response) => {
        this.branchOption = response.data.branchList
      })
    },
    selectBranch() {
      commit(this.form.projectId, this.form.branch).then((response) => {
        this.commitOption = response.data.commitList
      })
    },
    selectCommit() {
